...
public plugin_end()
{
	// Óíè÷òîæàåì ìàññèâ è äåðåâüÿ äëÿ îñâîáîæäåíèÿ ïàìÿòè
	if(get_pcvar_num(g_enableRanksCvar))
	{
...
public CmdReset(id, level, cid) // Ñáðîñ ñòàòû
{
...
format_statsme(id, motd[MAX_BUFFER_LENGTH])
{
	...
	for(i = 1 ; i < HLW_TANK; ++i) 
	{
		...
		{
			hl_get_wpnname(i, wpn, charsmax(wpn)) // Íàçâàíèå îðóæèÿ
			
			switch(i) // Äëÿ ëó÷øåé ñìîòðèáåëüíîñòè âûáèðàåì îòñòóï (êîë-âî ïðîáåëîâ ïîñëå íàçâàíèÿ îðóæèÿ)
			{
			...
			// Ñîáñòâåííî ñàì îòñòóï
			for(k = 0; k < t - 1; k++)
				add(wpn, charsmax(wpn), " ")
				
			// Ñòàòà ïî èñïîëüçîâàíîìó îðóæèþ
...
handle_report(id) // Ðåïîðò î ñîñòîÿíèè èãðîêà
{
	if(!SayReport)
	{
		client_print(id, print_chat, "%s %L", PREFIX, id, "DISABLED_MSG")
	...
	if(hl_is_user_spectator(id)) // Þçåð â ðåæèìå ñïåêòàòîðà
	{
	...
	new sBuffer[200]
	if(is_user_alive(id)) // Èãðîê æèâ
	{
		// Óçíàåì êîë-âî õåëñîâ è áðîíè, èä îðóæèÿ è åãî åíòèòè èä
		...
		// Ó èãðîêà â ðóêàõ îðóæèå, ïîëó÷àåì êîë-âî ïàòðîíîâ â îáîéìå è çàïàñå
		if(weapent > 0) 
		{
		...
		if(iClip >= 0) // Îðóæèå èìååò îáîéìó (ïèñòîëåòû è ò.ï)
		{
			switch(iWeapon)
			{
		...
		else // Îðóæèå áåç îáîéìû (ëîì è ò.ï.)
		{
			switch(iWeapon)
			{
...
public fw_PlayerKilled(victim, killer, shouldgib)
{
	...
	if(status == NORMAL)
	{
		...
		if(killer == inflictor) // Âû÷èñëÿåì ID îðóæèÿ
	...
	switch(weaponId) // Îáíîâëÿåì ñòàòèñòèêó îðóæèÿ
	{
		//case HLW_TANK: { }
...
public fwPlaybackEvent(flags, invoker, eventid) // Ñ÷èòàåì âûñòðåëû
{
	if (!(g_guns_eventids_bitsum & (1<<eventid)) || !(1 <= invoker <= g_maxPlayers))
		return FMRES_IGNORED	
	
	switch(eventid) // Ýâåíòû îáùèå äëÿ âñåõ ìîäîâ
	{
		case 1..2:
		{
...
hl_get_user_weapon_ent(client) // Âîçâðàùàåò ÈÄ ýíòèòè (weapon entityid) îðóæèÿ, êîòîðîå â äàííûé ìîìåíò ó èãðîêà
{
	return get_pdata_cbase(client, g_Offsets[ACTIVEITEM], EXTRAOFFSET)
}

hl_get_weapon_ammo(entity) // Âîçâðàùàåò êîë-âî ïàòðîíîâ â îáîéìå ó îðóæèÿ (íà âõîäå weapon entityid)
{
	return get_pdata_int(entity, g_Offsets[WEAPONCLIP], EXTRAOFFSET_WEAPONS)
}

hl_get_user_bpammo(client, weapon) // Âîçâðàùàåò êîë-âî ïàòðîíîâ â çàïàñå ó èãðîêà ïî ÈÄ îðóæèÿ (HLW_*)
{
...
bool:hl_is_user_spectator(client) // Èãðîê ñïåêòàòîð?
{
...
set_mod_offsets(MOD) // Çàãðóæàåì íóæíûå îôôñåòû
{
	new offset, i
	switch(MOD)
	{
		...
		{	
			...
			g_Offsets[ACTIVEITEM] = 350 // èëè 351 xD
		...
		case MINIAG: // ó ÀÃ è ìèíèÀÃ îäèíàêîâûå îôôñåòû =D
		{
...
RANKS_LOAD_FROM_FILE()
{
	...
	while(!feof(vault))
	{
		...
		else // Åñëè ôàéë ñòàòû îò ñòàðîé âåðñèè ïëàãèíà, òî ÷èòàåì åãî ÷óòîê ïî äðóãîìó
		{
...
RANKS_SAVE_TO_FILE() // Ñîõðàíÿåì ðàíêè â ôàéë
{
...
public RANKS_SORTING(Array:Uniqueid, Item1, Item2) // Ñîðòèðîâêà ðàíêîâ
{
...
handle_info(id) // Èíôîðìàöèÿ î ïëàãèíå
{
